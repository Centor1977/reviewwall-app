...
export default async function OffresPage() {
  ...
  if (prestataire) {
    const { data } = await supabase
      .from("offres")
      .select("id, titre, categorie, active, created_at, avis(id, note)")
        ...
        <div className="space-y-3">
          {list.map((offre) => {
            ...
            return (
              <div
                key={offre.id}
                className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-5 py-4 transition-shadow duration-150 hover:shadow-sm"
              >
                ...
                <Link
                  ...
                  className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition-colors duration-150 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
                >
                  Gérer
